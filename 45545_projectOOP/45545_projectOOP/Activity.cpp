...
Hour Activity::getStartTime()
{
	return startTime;
}
Hour Activity::getEndTime()
{
	return endTime;
}
void Activity::setName(char* oth)
{
	delete[] this->name;
	this->name = new char[strlen(oth) + 1];
	strcpy_s(name, strlen(oth) + 1, oth);
}
void Activity::setNote(char* oth)
{
	delete[] this->note;
	this->note = new char[strlen(oth) + 1];
	strcpy_s(note, strlen(oth) + 1, oth);
}
void Activity::setDateOfActivity(Date oth)
{
	this->dateOfActivity = oth;
}
void Activity::setStartTime(Hour oth)
{
	this->startTime = oth;
}
void Activity::setEndTime(Hour oth)
{
	this->endTime = oth;
}
Activity::Activity()
