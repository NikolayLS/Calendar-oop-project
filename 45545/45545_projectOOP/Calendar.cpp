...
#include <fstream>
//function that convert file with calendar to vector<Activity>
std::vector<Activity> Calendar::readFromFile(const char* fileName)
{
...
void Calendar::saveAs(const char* fileName) const
{
	Calendar temp = *this;
	char* intHelper;
	int len;
...
void Calendar::findSlot(Date othDate, Hour othHour) const
{
...
void Calendar::busyDays(Date from, Date to) const 
{
	Date arr[100];
	int br = 0;
	Date plus(0, 0, 1);
...
//function that helps "busyDays"-funtion by retuning busy hours of day
Hour Calendar::busyOnDate(Date oth) const
{
	Hour plus;
...
// help function that finds if you can book an activity
bool Calendar::canIBook(Activity oth) const
{
	for (int i = 0;i < size;i++)
	{
...
void Calendar::find(const char* oth) const
{
...
void Calendar::change(Date othDate, Hour othHour, const char* changeStr)
{
	Activity temp;
	for (int i = 0;i < size;i++)
	{
...
void Calendar::agenda(Date oth) const
{
	int j = 0;
	Activity* temp;
...
//funciton that let us to see every activity in our calendar
void Calendar::showAll()const
{
	for (int i = 0;i < size;i++)
	{
...
void Calendar::findSlotWith(const char* nameOfCalendar, Date fromDate, Hour hours)
{
	Calendar tempCalendar;
...
void Calendar::merge(const char* otherCalendar)
{
	Calendar tempCalendar;
	std::vector<Activity> tempCalendarHelper;
